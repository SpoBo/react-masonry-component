...
var MasonryComponent = React.createClass({
  ...
  diffDomChildren: function() {
    ...
    if (this.heights && !removed.length && !appended.length && !prepended.length && !moved.length) {
      heightChanged = this.heights.some(function(oldHeight, index) {
        return oldHeight !== newChildren[index].clientHeight;
      });
    }
  ...
  performLayout: function() {
    ...
    if (diff.heightChanged || (diff.removed.length || diff.appended.length || diff.prepended.length || diff.moved.length)) {
      this.masonry.layout();
    }
  },
  ...
  storeHeights: function() {
    this.heights = this.getNewDomChildren().map(function(child) {
